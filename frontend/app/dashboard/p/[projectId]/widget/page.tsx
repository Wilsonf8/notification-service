 ...
 * @module app/dashboard/p/[projectId]/widget/page
 */
"use client";
...
export default function WidgetPage() {
  ...
  const [offlineMessage, setOfflineMessage] = useState("No reps available. Leave your info and we'll get back to you.");
        ...
        <div className={cn("space-y-4 lg:col-span-3", customizationDisabled && "pointer-events-none opacity-50")}>
          ...
          <Card>
            ...
            <CardContent className="space-y-5">
              {renderColorPicker("Accent Color", "The main color for buttons and interactive elements", widgetColor, setWidgetColor)}
              {renderColorPicker("Background Color", "Widget panel background", backgroundColor, setBackgroundColor)}
              {renderColorPicker("Text Color", "Text and label color", textColor, setTextColor)}
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Choose the icon displayed on the floating button</CardDescription>
            </CardHeader>
          ...
          <Card>
            ...
            <CardContent className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                <Select value={fontFamily} onValueChange={(v) => setFontFamily(v as WidgetFontFamily)}>
          ...
          <Card>
            ...
            <CardContent className="space-y-5">
              <div className="space-y-2">
                ...
                <Textarea id="welcomeMessage" value={welcomeMessage} onChange={(e) => setWelcomeMessage(e.target.value)} maxLength={500} rows={2} />
                <p className="text-xs text-muted-foreground">Shown when the widget panel opens. Max 500 chars.</p>
              </div>
              <div className="space-y-2">
                <Label htmlFor="offlineMessage">Offline Message</Label>
                <Textarea id="offlineMessage" value={offlineMessage} onChange={(e) => setOfflineMessage(e.target.value)} maxLength={500} rows={3} />
                <p className="text-xs text-muted-foreground">Shown when no reps are online. Max 500 chars.</p>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="lg:col-span-2">
          <div className="sticky top-6">
            <Card>
              ...
              <CardContent>
                <div className="relative overflow-hidden border border-border" style={{ height: 480 }}>
                  <div className="absolute inset-0" style={{ backgroundColor: "#1a1a1a", backgroundImage: "radial-gradient(circle, #333 1px, transparent 1px)", backgroundSize: "20px 20px" }} />
                  <div className="absolute inset-0">
                    {previewExpanded && (
                      ...
                      >
                        <div style={{ padding: "10px 14px", display: "flex", alignItems: "center", justifyContent: "space-between", backgroundColor: `color-mix(in srgb, ${backgroundColor}, white 8%)`, borderBottom: `1px solid color-mix(in srgb, ${backgroundColor}, white 14%)` }}>
                          <div className="flex items-center gap-2">
                            <IconCircleFilled className="h-2.5 w-2.5" style={{ color: "#22c55e" }} />
                            <span style={{ fontSize: 12, fontWeight: 600, color: textColor }}>Online</span>
                          </div>
                          <button type="button" onClick={() => setPreviewExpanded(false)} style={{ background: "none", border: "none", cursor: "pointer", padding: 2, display: "flex", color: textColor, opacity: 0.6 }}>
                            <IconX className="h-4 w-4" />
                          </button>
                        </div>
                        <div style={{ padding: 14 }}>
                          <p style={{ fontSize: 13, color: textColor, marginBottom: 14, lineHeight: 1.4 }}>
                            {welcomeMessage || "Hi! How can we help you today?"}
                          </p>
                          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                            <button type="button" style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 12px", backgroundColor: widgetColor, color: backgroundColor, border: "none", borderRadius: `${borderRadius}px`, cursor: "default", fontSize: 12, fontWeight: 500, fontFamily: "inherit" }}>
                              <IconVideo className="h-4 w-4" />
                              Start Video Call
                            </button>
                            <button type="button" style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 12px", backgroundColor: "transparent", color: textColor, border: `1px solid color-mix(in srgb, ${backgroundColor}, white 14%)`, borderRadius: `${borderRadius}px`, cursor: "default", fontSize: 12, fontWeight: 500, fontFamily: "inherit" }}>
                    ...
                    >
                      {previewExpanded ? (
                        <IconX className="h-5 w-5" style={{ color: backgroundColor }} />
                      ) : (
                        (() => {
                          const FabIcon = ICON_MAP[widgetIcon] || IconVideo;
                          return <FabIcon className="h-5 w-5" style={{ color: backgroundColor }} />;
                        })()
                      )}
                    </button>
                  </div>
