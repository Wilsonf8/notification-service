...
public interface ProjectRepository extends JpaRepository<Project, UUID> {
    ...
    @Query("SELECT p FROM Project p JOIN OrganizationMember m ON m.organization.id = p.organization.id WHERE p.id = :projectId AND m.user.id = :userId AND p.deletedAt IS NULL")
